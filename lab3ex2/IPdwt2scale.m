...
for i = 1 : j
  ...
  out(1:mid_r, mid_c:initcol) = (w .- min(min(w))) * (255 / (max(max(w)) - min(min(w))));

  w = out(mid_r:initrow, 1:mid_c);
  out(mid_r:initrow, 1:mid_c) = (w .- min(min(w))) * (255 / (max(max(w)) - min(min(w))));

  w = out(mid_r:initrow, mid_c:initcol);
  out(mid_r:initrow, mid_c:initcol) = (w .- min(min(w))) * (255 / (max(max(w)) - min(min(w))));
